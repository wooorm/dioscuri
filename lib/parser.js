...
/**
 * @typedef {'ascii' | 'base64' | 'binary' | 'hex' | 'latin1' | 'ucs-2' | 'ucs2' | 'utf16le' | 'utf-8' | 'utf8'} BufferEncoding
 ...
 *   Acceptable input.
 *
 * @typedef {'eof' | 'eol' | 'headingSequence' | 'headingText' | 'linkSequence' | 'linkText' | 'linkUrl' | 'listSequence' | 'listText' | 'preAlt' | 'preSequence' | 'preText' | 'quoteSequence' | 'quoteText' | 'text' | 'whitespace'} Type
 *   Token type.
 *
 * @typedef Token
 *   Base token.
...
/**
 * @returns {(value: Value | null | undefined, encoding?: BufferEncoding | null | undefined, end?: boolean | null | undefined) => Array<Token>}
 */
export function parser() {
   ...
   * Parse a chunk.
   *
   * @param {Value | null | undefined} buf
   * @param {BufferEncoding | null | undefined} [encoding]
   * @param {boolean | null | undefined} [done=false]
   * @returns {Array<Token>}
   */
  function parse(buf, encoding, done) {
    ...
    if (buf) values.push(buf.slice(start).toString(encoding || undefined))
